...
#     ITENS_PROVA_ENEM_2016.csv no diretório raiz. 
#     Ex. Windows C:\
#         Linux \home	                  
#------------------------------------------------------------------------
...
#itens_2016$SG_AREA <- factor(itens_2016$SG_AREA, levels = c('CH','CN','LC','MT'),  labels=c('Ciências Humanas','Ciências da Natureza','Linguagens e Códigos','Matemática'))
#itens_2016$TP_LINGUA <- factor(itens_2016$TP_LINGUA, levels = c(0,1),  labels=c('Inglês','Espanhol'))
#itens_2016$IN_ITEM_ADAPTADO <- factor(itens_2016$IN_ITEM_ADAPTADO, levels = c(1,0),  labels=c('Sim','Não'))
#itens_2016$IN_ITEM_ABAN <- factor(itens_2016$IN_ITEM_ABAN, levels = c(1,0),  labels=c('Sim','Não'))
